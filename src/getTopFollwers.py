...
'''
        ...
        try:
            for follower in res["nodes"]:
                ...
                active = (
                    follower["contributionsCollection"]["contributionCalendar"][
                        "totalContributions"
                    ]
                    > 5
                )
                if not active:
                    print(
                        f"Skipped{'*' if followerNumber > 500 else ''} (inactive): https://github.com/{login} with {followerNumber} followers and {following} following"
                    )
                    continue
                quota = followerNumber
                for i, starCount in enumerate(
                    [
                        repo["stargazerCount"]
                        for repo in follower["repositories"]["nodes"]
                    ]
                ):
                    if starCount <= i:
                        break
                ...
                for i, starCount in enumerate(
                    [
                        repo["stargazerCount"]
                        for repo in follower["repositoriesContributedTo"]["nodes"]
                    ]
                ):
                    if starCount <= i:
                        break
                ...
                if following > quota:
                    print(
                        f"Skipped{'*' if followerNumber > 500 else ''} (quota): https://github.com/{login} with {followerNumber} followers and {following} following"
                    )
                    continue
